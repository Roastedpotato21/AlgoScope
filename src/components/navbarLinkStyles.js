export const getNavLinkClasses = ({
  ...
  if (isActive) {
    base.push(
      'bg-indigo-500/10 text-indigo-600 shadow-[0_18px_40px_rgba(99,102,241,0.12)]'
    )
    base.push(
      'before:scale-x-100 before:bg-indigo-500 before:transition-transform before:duration-500'
    )
    if (isDark) {
      base.push(
        'dark:bg-indigo-500/15 dark:text-indigo-300 dark:shadow-[0_18px_40px_rgba(99,102,241,0.18)]'
      )
    }
    return base.join(' ')
  }
  ...
  if (isDark) {
    base.push(
      'dark:text-slate-300 dark:hover:text-white dark:hover:bg-slate-900/70'
    )
  }
